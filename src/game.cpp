...
void Game::displayTowerInfos(SDL_Surface* dest,const towerBase* pTower) const
{
	...
	font->drawSolid(dest,pos,"Cost  : "+to_string(pTower->getCostMoney())+" $");	pos.add(row);
	font->drawSolid(dest,pos,"Range : "+to_string(pTower->getRange())+" m");		pos.add(row);
	font->drawSolid(dest,pos,"Damage: "+pTower->getDamageInfo());					pos.add(row);
	//font->drawSolid(dest,pos,pTower->getUpgradeInfo());

	pos=mTowerCellArea.getTopLeft();
	font->drawSolid(dest,pos,"Sell : "+to_string(pTower->getSellValue())+" $");
	/*int iThick=4;
...
void Game::printScores(SDL_Surface* p)
{
	...
	if (state()==STATE_OVER)
	{
		mpoFontHudge->drawSolidShadowed(back,coord(20,150),"GAME OVER");
	}
	if (state()==STATE_PAUSED)
	{
		mpoFontHudge->drawSolidShadowed(back,coord(20,150),"GAME PAUSED");
	}
}
